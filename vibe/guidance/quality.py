...
def _get_action_words() -> set[str]:
    """Get set of action words for command detection."""
    return {
        "run",
        "execute",
        "create",
...
def _calculate_text_metrics(message: str) -> dict[str, Any]:
...
def _check_action_word_start(words: list[str]) -> bool:
    """Check if text starts with an action word."""
    action_words = _get_action_words()
...
def _analyze_text_quality(message: str) -> dict[str, Any]:
    ...
    metrics = _calculate_text_metrics(message)
    starts_with_action = _check_action_word_start(metrics["words"])
    ...
    return {
        ...
        "complexity_score": (
            metrics["avg_syllables_per_word"] * metrics["avg_sentence_length"]
        ),
    }
...
def _add_primary_keys(ordered: OrderedDict[str, Any], data: dict[str, Any]) -> None:
    """Add primary workflow keys in preferred order."""
    for key in (
        "name",
        "description",
        "triggers",
...
def _add_extra_metadata_keys(
    ordered: OrderedDict[str, Any], data: dict[str, Any]
) -> None:
    """Add known extra metadata keys in sorted order."""
    for key in sorted(ALLOWED_EXTRA_KEYS):
        if key in data:
            ordered[key] = data[key]


def _add_remaining_keys(ordered: OrderedDict[str, Any], data: dict[str, Any]) -> None:
    """Add any remaining keys to avoid destructive changes."""
    for key in data:
...
def _normalize_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    _add_primary_keys(ordered, data)
    _add_extra_metadata_keys(ordered, data)
    _add_remaining_keys(ordered, data)
